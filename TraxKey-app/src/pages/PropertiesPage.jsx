...
function AddUnitForm({ propertyId, onCreated, onClose }) {
  ...
  return (
    <form onSubmit={submit} className="flex flex-wrap items-center gap-2 bg-slate-950/60 border border-white/5 rounded-lg p-3 mt-2">
      <input placeholder="Unit # (blank for single-family)" value={form.unitNumber} onChange={update('unitNumber')}
        className="bg-slate-900 border border-white/10 rounded-lg px-3 py-1.5 text-xs w-48 focus:outline-none focus:border-teal-400" />
      <input placeholder="Beds" type="number" min="0" value={form.bedrooms} onChange={update('bedrooms')}
        className="bg-slate-900 border border-white/10 rounded-lg px-3 py-1.5 text-xs w-20 focus:outline-none focus:border-teal-400" />
      ...
      <button type="button" onClick={onClose} className="text-xs text-slate-400 hover:text-white">Cancel</button>
    ...
    <div className="bg-slate-900 border border-white/5 rounded-2xl p-5 mb-4">
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          <p className="text-xs text-slate-500">{property.address_line1}{property.address_line2 ? `, ${property.address_line2}` : ''}, {property.city}, {property.state} {property.zip}</p>
        </div>
        <span className="text-[10px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-full bg-teal-500/15 text-teal-400 shrink-0">
          {property.property_type.replace('_', ' ')}
        </span>
      </div>

      <div className="space-y-1.5">
        {property.units.length === 0 && <p className="text-xs text-slate-500">No units yet.</p>}
        {property.units.map(u => (
          <div key={u.id} className="flex items-center gap-3 text-sm bg-slate-950/40 rounded-lg px-3 py-2">
            <span className="font-medium">{u.unit_number || 'Whole property'}</span>
            <span className="text-slate-500 text-xs">{u.bedrooms ?? '–'} bd / {u.bathrooms ?? '–'} ba</span>
      ...
      {addingUnit ? (
        ...
        <button onClick={() => setAddingUnit(true)} className="text-xs text-teal-400 hover:text-teal-300 mt-3">+ Add unit</button>
      )}
    </div>
  );
}
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 px-6 py-8">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            <p className="text-xs text-teal-400 font-semibold uppercase tracking-wide mb-1">Properties</p>
        ...
        {!properties && !error && <p className="text-sm text-slate-500">Loading…</p>}
        {properties && properties.length === 0 && (
          <div className="bg-slate-900 border border-white/5 rounded-2xl p-8 text-center">
            <p className="text-sm text-slate-400">No properties yet. Add your first one above, this is what TraxKey AI's agents will monitor and act on.</p>
          </div>
        )}
