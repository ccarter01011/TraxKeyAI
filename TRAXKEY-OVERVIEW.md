...
| **Orders & Suppliers** | Parts and materials a job is waiting on, flagged when late, and when late means a turn will slip. Suppliers are real records (name, contact email, phone, notes), not retyped free text on every order - added 2026-08-20, same shape as invoice customers. Each supplier carries a default contact email, CC, and auto-chase setting; a per-order override inherits it when left blank. On-time rate is computed from this account's own order history per supplier, never an industry benchmark. Adapted from our own supply-chain product, TraxSail AI, which chases suppliers on purchase orders the same way. |
...
| Turnover density and lease-expiry flags on the ops calendar | How many units share a turn deadline the same day (cleaners are a shared resource, so this is a bottleneck signal) and a visible lease-expiring-soon marker, without needing to hover each cell. Raised 2026-08-17, not yet scoped. |
| Public booking page per customer | A path-based, dynamic page (`traxkey.ai/book/company-slug`, not a subdomain or a static file per customer) so a PM or STR operator can promote one link for availability and direct reservations before they have their own site — reads live from the same units/pricing data the operator dashboard uses. Scheduled after Stripe subscription billing (§11) since it's the first public, unauthenticated surface that can **write** (create a reservation), which needs its own scoping — company fixed by the slug and never client-controlled, rate limiting, no leakage of internal ops data — before it ships. Raised 2026-08-17, not yet scoped. |
